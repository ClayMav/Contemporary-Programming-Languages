...
then
    ...
    echo ""
    echo "If this was a Node project, we'd just run \"npm install\" to set things up."
    echo "However, that's a little unnecessary, since all we want is ESLint."
    echo ""
    echo "I'll go ahead and use npm to install it right now."
    echo ""

    npm install eslint@4.8.0 eslint-config-google@0.9.1
fi
